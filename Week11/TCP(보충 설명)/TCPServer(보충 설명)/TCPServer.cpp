...
int main(int argc, char* argv[])
{
	...
	serveraddr.sin_port = htons(9000);				// sin_port : 포트 번호(접속 시 새로운 소켓 포트 번호가 생성), htons() : short intger 데이터를 호스트 byte order로 변경
	...
	while(1){
		...
		printf("\n[TCP 서버] 클라이언트 접속: IP 주소=%s, 포트 번호=%d\n", inet_ntoa(clientaddr.sin_addr), ntohs(clientaddr.sin_port));	// 접속, inet_ntoa() : 정수 형태를 문자열 형태로 변환, ntohs() : short intger 데이터를 호스트 byte order로 변경
